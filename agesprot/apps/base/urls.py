...
urlpatterns = patterns('agesprot.apps.base.views',
	...
	url(r'^list-roles-project/$', permission_required('is_superuser')(ListRolesProjectView.as_view()), name = 'list_roles_project'),
	url(r'^form-role/(?:/(?P<role_pk>\d+))?', 'form_role', name = 'form_role'),
	url(r'^delete-role/(?P<role_pk>\d+)/$', 'delete_role', name = 'delete_role'),
	url(r'^list-prioridad-project/$', permission_required('is_superuser')(ListPrioridadProjectView.as_view()), name = 'list_prioridad_project'),
